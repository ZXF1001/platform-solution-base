...
- [网页平台相关操作记录](#网页平台相关操作记录)
  - [CentOS服务器配置相关](#centos服务器配置相关)
    - [1. 网站服务器的开启（使用Linux+Nginx+MySQL+PHP即lnmp）](#1-网站服务器的开启使用linuxnginxmysqlphp即lnmp)
    - [2. 更改跨域访问设置](#2-更改跨域访问设置)
    - [3. 开放端口方法](#3-开放端口方法)
    - [4. Nginx设置虚拟目录托管静态文件](#4-nginx设置虚拟目录托管静态文件)
    - [5. Nginx托管api接口](#5-nginx托管api接口)
    - [6. Nginx托管websocket接口](#6-nginx托管websocket接口)
  - [数据库相关](#数据库相关)
    - [1. 数据库忘记密码](#1-数据库忘记密码)
    - [2. 数据库创建只读用户](#2-数据库创建只读用户)
    - [3. 建表的SQL语句](#3-建表的sql语句)
    - [4. 查询相关的SQL语句](#4-查询相关的sql语句)
  - [服务器后端api的开发](#服务器后端api的开发)
    - [1. 常用框架：](#1-常用框架)
    - [2. Express框架搭建](#2-express框架搭建)
    - [3. WebSocekt接口实现](#3-websocekt接口实现)
    - [4. 服务如何常驻、终止](#4-服务如何常驻终止)
  - [Git相关](#git相关)
    - [1. Github被墙时，命令行Git提交的方法](#1-github被墙时命令行git提交的方法)
